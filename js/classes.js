...
import { escapeHtml, numberOrNull, setStatus, tagCheckboxesHtml } from './dashboard-shared.js';
...
async function init() {
  ...
  renderSidebar({ activePage: 'classes', user: ctx.user });
  currentStudio = ctx.studio;

  await Promise.all([loadTags(), loadClasses()]);
}
